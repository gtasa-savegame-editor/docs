import * as React from 'react';
import { FunctionComponent } from 'react';

type Props = {}

const Garages: FunctionComponent<Props> = () => {
  return (
      <div>
        <div className="section">
          <h2>Garages</h2>
          <p>
            This table shows all the available information regarding garages.
          </p>
          <table>
            <tbody>
            <tr>
              <th className="header int">ID</th>
              <th className="header float">X</th>
              <th className="header float">Y</th>
              <th className="header float">Z</th>
              <th className="header">Name</th>
              <th className="header">Readable Name</th>
              <th className="header">Notes</th>
            </tr>
            <tr>
              <td className={'int'}>16</td>
              <td className={'float'}>2502.31</td>
              <td className={'float'}>-1699.36</td>
              <td className={'float'}>12.4323</td>
              <td>cjsafe</td>
              <td>Ganton garage (Grove St.)</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>17</td>
              <td className={'float'}>319.326</td>
              <td className={'float'}>-1768.93</td>
              <td className={'float'}>3.35686</td>
              <td>beacsv</td>
              <td>Santa Maria Beach garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>18</td>
              <td className={'float'}>2449.5</td>
              <td className={'float'}>695.018</td>
              <td className={'float'}>10.4742</td>
              <td>vEsvgrg</td>
              <td>Rockshore West garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>24</td>
              <td className={'float'}>-364.439</td>
              <td className={'float'}>-1194.37</td>
              <td className={'float'}>-18.597</td>
              <td>cn2gar1</td>
              <td>Fort Carson garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>25</td>
              <td className={'float'}>430.059</td>
              <td className={'float'}>2542.31</td>
              <td className={'float'}>15.166</td>
              <td>cn2gar2</td>
              <td>Verdant Meadows garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>26</td>
              <td className={'float'}>783.155</td>
              <td className={'float'}>-492.75</td>
              <td className={'float'}>16.3361</td>
              <td>burbdo2</td>
              <td>Dillimore garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>27</td>
              <td className={'float'}>1269.2</td>
              <td className={'float'}>2525.14</td>
              <td className={'float'}>9.80013</td>
              <td>blob69</td>
              <td>Prickle Pine garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>28</td>
              <td className={'float'}>929.554</td>
              <td className={'float'}>2008.59</td>
              <td className={'float'}>10.115</td>
              <td>blob7</td>
              <td>Whitewood Estates garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>29</td>
              <td className={'float'}>2227.6</td>
              <td className={'float'}>168.649</td>
              <td className={'float'}>26.4635</td>
              <td>burbdoo</td>
              <td>Palomino Creek garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>30</td>
              <td className={'float'}>1408.65</td>
              <td className={'float'}>1899.52</td>
              <td className={'float'}>10.115</td>
              <td>blob6</td>
              <td>Redsands West garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>31</td>
              <td className={'float'}>1694.8</td>
              <td className={'float'}>-2088.7</td>
              <td className={'float'}>12.3636</td>
              <td>carlas1</td>
              <td>El Corona garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>32</td>
              <td className={'float'}>1352.58</td>
              <td className={'float'}>-636.657</td>
              <td className={'float'}>108.135</td>
              <td>CEsafe1</td>
              <td>Muholland garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>33</td>
              <td className={'float'}>1523.92</td>
              <td className={'float'}>-1653.23</td>
              <td className={'float'}>4.72837</td>
              <td>imp_la</td>
              <td>Los Santos impound lot</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>34</td>
              <td className={'float'}>-1652.78</td>
              <td className={'float'}>647.502</td>
              <td className={'float'}>-6.04924</td>
              <td>imp_sf</td>
              <td>San Fierro impound lot</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>35</td>
              <td className={'float'}>2218.06</td>
              <td className={'float'}>2448.06</td>
              <td className={'float'}>-8.43807</td>
              <td>imp_lv</td>
              <td>Las Venturas impound lot</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>36</td>
              <td className={'float'}>2640.78</td>
              <td className={'float'}>-2049.99</td>
              <td className={'float'}>12.543</td>
              <td>modlast</td>
              <td>Loco Low Co</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>37</td>
              <td className={'float'}>-2728.53</td>
              <td className={'float'}>-212.295</td>
              <td className={'float'}>-3.45112</td>
              <td>mds1SFS</td>
              <td>Wheels Arch Angel</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>39</td>
              <td className={'float'}>-2108.92</td>
              <td className={'float'}>-886.553</td>
              <td className={'float'}>-75.566</td>
              <td>sav1sfe</td>
              <td>Calton Heights garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>40</td>
              <td className={'float'}>-2699.12</td>
              <td className={'float'}>-821.489</td>
              <td className={'float'}>-49.0042</td>
              <td>sav1sfw</td>
              <td>Paradiso garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>41</td>
              <td className={'float'}>-2043.1</td>
              <td className={'float'}>-118.609</td>
              <td className={'float'}>27.821</td>
              <td>hbgdSFS</td>
              <td>Main Doherty garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>42</td>
              <td className={'float'}>-2454.02</td>
              <td className={'float'}>-131.556</td>
              <td className={'float'}>25.0886</td>
              <td>svgsfs1</td>
              <td>Hashbury garage</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>44</td>
              <td className={'float'}>1550.98</td>
              <td className={'float'}>1155.36</td>
              <td className={'float'}>8.97329</td>
              <td>vgshngr</td>
              <td>AT 400 hangar</td>
              <td></td>
            </tr>
            <tr>
              <td className={'int'}>45</td>
              <td className={'float'}>383.843</td>
              <td className={'float'}>2433.28</td>
              <td className={'float'}>15.166</td>
              <td>dhangar</td>
              <td>Verdant Meadows hangar</td>
              <td></td>
            </tr>
            </tbody>
          </table>
          <p>
            There are also some garages without ids, apparently those can't be
            used in savegames.
          </p>
        </div>
      </div>
    );
}

export default Garages;
